...
checkDockerVersion() {
  ...
  if [ "${DOCKER_VERSION}0" -lt "${MIN_DOCKER_VERSION//./}" ]; then
    echo "Docker version is ${DOCKER_VERSION} below the minimum required version ${MIN_DOCKER_VERSION//./}"
...
IMAGE_NAME="oracle/autonomousdatabase:$VERSION"
